...
% The evaluation criteria may depend on the estimand. When descriptive statistics are the goal and when statistical inference would not be of interest, bias of the estimates would still apply, but standard errors are generally ignored.

% Some simulators may focus on retrieving the unobserved values, while others study the performance in the context of obtaining efficient predictions or classification.  
% For many outcome-driven imputation tasks a single valid imputation may perhaps suffice. 
...
The imputation methods under evaluation should be applied to the generated incomplete data, after which an analysis model may be fitted to estimate the estimand(s). % It is important to choose the imputation model parameters carefully (e.g., number of imputations, number of iterations, regularization, etc.). After imputation, an analysis model may be fitted to estimate the estimand(s).
...
% A minimum requirement for any imputation method is the absence of non-convergence in the imputation-generating process. If non-convergence is suspected, the inference resulting from the imputations might be invalid. 
...
% [Be explicit in choices. Describe the simulation textually, use pseudo-code, a flowchart, visualize missingness and simulation results. For example, the procedure to generate missing data is often insufficiently described in publications. The used missingness mechanism should be detailed, either graphically or written as a function of the data. Fill in the following checklist (in appendix).]
...
% \subsection*{A.1.\enspace Simulation setup}

% Let $Y=(Y_{obs},Y_{mis})$ be an incomplete semicontinuous variable, where $Y_{obs}$ and $Y_{mis}$ denote the observed values and the missing values in $Y$, respectively. Further, $X=(X_1,\dots,X_k)$ is a set of $k$ fully observed covariates, where $X_{obs}$ and $X_{mis}$ correspond to the observed an missing parts in $Y$ and where $X_j$ would indicate the $j$th variable in $X$, with $j=1,\dots, k$. Finally, let $R$ be a response indicator that is 1 if $Y$ is observed and 0 if $Y$ is missing.
% 
% To prove the point, we generated a single data set from the multivariate normal distribution with means
...
% \end{equation*}
% 
% Three incomplete sets were then generated, wherein random missingness was imposed in $Y$ in three ways. First, we left the observed data \emph{observed at random} to impose MCAR missingness following the mechanism 
% \begin{equation*}
