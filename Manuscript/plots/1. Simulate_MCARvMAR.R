...
compl <- data.frame(mvrnorm(n     = 500, 
                            mu    = c(5, 10, 10), 
...
for(i in 1:nsim){
  output.MCAR[[i]] <- simulate.mis(compl[, -3], "MCAR", .5) #exclude tp conform to 2 dims
  output.MAR1[[i]] <- simulate.mis(compl[, -3], "MARright", .5) #high correlation
  output.MAR2[[i]] <- simulate.mis(compl[, -2], "MARright", .5) #low correlation
